...
| 12 | Define narrative bible v0 for Road Master and Chapter I | 0.0.0 Foundations | partial | `apps/road-master/docs/foundations/doctrine.md`, `lane/spark-5-narrative-shell`, `58fb0fd` |
...
| 16 | Annotate seed dataset of 50-100 questions for one region | 0.0.1 Content Annotation Prototype | documented | `apps/road-master/data/chapter-1-crossing-fields/questions.json`, `lane/spark-2-content` |
| 17 | Build internal content editor for inspection and edits | 0.0.1 Content Annotation Prototype | documented | `apps/road-master/tools/content-inspector.mjs`, `lane/spark-2-content` |
...
| 32 | Epic: 0.0.7 telemetry and balancing | 0.0.7 Telemetry And Balancing | partial | `apps/road-master/docs/foundations/telemetry.md`, `lane/spark-1-foundations` |
| 33 | Wire telemetry dashboards or core balancing queries for the vertical slice | 0.0.7 Telemetry And Balancing | partial | `apps/road-master/docs/foundations/telemetry.md`, `lane/spark-1-foundations` |
| 34 | Tune vertical-slice metrics using completion, retry, fail and reclaim data | 0.0.7 Telemetry And Balancing | partial | `apps/road-master/docs/foundations/telemetry.md`, `lane/spark-1-foundations` |
...
| 38 | Epic: 0.0.9 MVP hardening | 0.0.9 MVP Hardening | documented | `apps/road-master/docs/foundations/release-gates.md`, `apps/road-master/docs/foundations/contracts-freeze.md` |
| 39 | Implement onboarding, persistence and first-time user path | 0.0.9 MVP Hardening | documented | `apps/road-master/docs/foundations/release-gates.md`, `apps/road-master/docs/foundations/contracts-freeze.md` |
| 40 | Run MVP feature freeze and hardening pass | 0.0.9 MVP Hardening | documented | `apps/road-master/docs/foundations/release-gates.md`, `apps/road-master/docs/foundations/contracts-freeze.md` |
| 41 | Epic: 0.1.0 MVP | 0.1.0 MVP | documented | `lane/spark-2-content` commit `f505b9b`, `58fb0fd`, `apps/road-master/tools/content-inspector.mjs` |
...
| 45 | Implement memory mechanics for known-ground slips and reclaim | 0.1.0 MVP | documented | `lane/spark-4-map-memory` commit `faff3af`, `apps/road-master/src/systems/memory/*` |
| 46 | Add narrative and audiovisual pass for Chapter I | 0.1.0 MVP | documented | `lane/spark-5-narrative-shell` commit `332089c`, `58fb0fd` |
| 47 | Balance and harden 0.1.0 for release candidate | 0.1.0 MVP | documented | `apps/road-master/docs/foundations/release-gates.md`, `apps/road-master/docs/roadmap/roadmap-assumptions-and-hardening.md` |
| 48 | Epic: 0.2.0 more content, same thesis | 0.2.0 More Content | roadmap-only | Roadmap manifest plus `0.1.0` pack baseline |
| 49 | Epic: 0.3.0 combat depth | 0.3.0 Combat Depth | roadmap-only | Roadmap manifest plus `lane/spark-3-combat-pacing` baseline |
| 50 | Epic: 0.4.0 better pedagogy without softness | 0.4.0 Pedagogy Depth | roadmap-only | Roadmap manifest plus `lane/spark-2-content` baseline |
| 51 | Epic: 0.5.0 map recursion and deeper memory palace | 0.5.0 Memory Palace Expansion | roadmap-only | Roadmap manifest plus `lane/spark-4-map-memory` baseline |
| 52 | Epic: 0.6.0 personalization | 0.6.0 Personalization | roadmap-only | Roadmap manifest plus release-gates docs |
| 53 | Epic: 0.7.0 social and shareable status | 0.7.0 Social Layer | roadmap-only | Roadmap manifest plus doctrine non-goals |
| 54 | Epic: 0.8.0 predictive and institutional layer | 0.8.0 Predictive And B2B | roadmap-only | Roadmap manifest plus telemetry docs |
| 55 | Epic: 0.9.0 product hardening | 0.9.0 Product Hardening | roadmap-only | Roadmap manifest plus release-gates and contracts-freeze docs |
| 56 | Epic: 1.0.0 full product | 1.0.0 Full Product | roadmap-only | Roadmap manifest plus all completed lane evidence |
| 57 | Cross-cutting epic: stable concept graph and trap taxonomy | 1.0.0 Full Product | roadmap-only | Roadmap manifest plus foundations and content evidence |
| 58 | Cross-cutting epic: curriculum and world expansion | 1.0.0 Full Product | roadmap-only | Roadmap manifest plus chapter pack baseline |
| 59 | Cross-cutting epic: analytics, readiness and fail-risk modeling | 1.0.0 Full Product | roadmap-only | Roadmap manifest plus telemetry evidence |
| 60 | Cross-cutting epic: narrative arc, chapters and final gate | 1.0.0 Full Product | roadmap-only | Roadmap manifest plus narrative shell evidence |

## Reading the matrix

The lower rows are not pretending to be done. They show which local artifacts already exist and where the roadmap still needs real implementation. That is enough to defend the slice without inflating completion claims.
